...
namespace API.Core
{
    public class JwtTokenCreator
    {
        ...
        public JwtTokenCreator(UpWorkContext context, JwtSettings settings, ITokenStorage storage)
        {
            _context = context;
            _settings = settings;
            _storage = storage;
        ...
        public string Create(string email, string password)
        {
            ...
            if (!BCrypt.Net.BCrypt.Verify(password + "UpWork", user.Password))
            {
                throw new UnauthorizedAccessException();
            }
            ...
            var claims = new List<Claim>
            {
                 ...
                 new Claim("Connects", user.Connects.ToString()),
